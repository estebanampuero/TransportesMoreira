    ...
    <div className="fleet-card animate-pulse">
      ...
      <div className="p-6 flex flex-col gap-4">
        ...
        <div className="space-y-2">
          ...
          <div className="h-3 w-3/5 bg-slate-800/60 rounded" />
        </div>
        <div className="flex gap-2 flex-wrap">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-6 w-24 bg-slate-800/60 rounded-full" />
          ))}
        </div>
...
// ─── Media area ─────────────────────────────────────────────────────────────
function MediaArea({
  coverImage, coverVideo, truckType, capacity,
}: {
  coverImage?: string
  coverVideo?: string
  truckType: string
    ...
    >
      {hasVideo ? (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-950 gap-3">
          <span className="text-6xl opacity-20 group-hover:opacity-35 transition-opacity duration-500 select-none">
            {icon}
          </span>
...
function FleetCard({ driver, delay }: { driver: FleetDriver; delay: number }) {
  ...
  return (
    ...
    >
      <MediaArea
        coverImage={driver.coverImage}
        coverVideo={driver.coverVideo}
      ...
      <div className="p-6 flex flex-col gap-4 flex-1">
        ...
        <div>
          ...
          {driver.truckPlate && (
            <p className="text-slate-500 text-sm font-mono tracking-wide mt-0.5">{driver.truckPlate}</p>
          )}
        </div>
        ...
        {driver.features && driver.features.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {driver.features.map((f) => (
              <span
                key={f}
                className="inline-flex items-center gap-1 text-xs text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-full"
              >
        ...
        <div className="mt-auto pt-2">
          <a
            href={buildWhatsAppUrl(waMsg, 'fleet_card')}
            target="_blank"
...
export default function Fleet() {
  ...
  useEffect(() => {
    // Only shows drivers the admin has explicitly marked showInFleet: true
    const q = query(collection(db, 'drivers'), where('showInFleet', '==', true))
    const unsub = onSnapshot(
      q,
      (snap) => {
        const data = snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<FleetDriver, 'id'>) }))
        setDrivers(data)
        setLoading(false)
      },
      () => setLoading(false),
  ...
  return (
    <section id="flota" className="section-alt" aria-labelledby="flota-title">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Bottom CTA strip — only shown when there are cards */}
        {!loading && drivers.length > 0 && (
          <div className={`mt-14 fleet-cta-strip rounded-2xl p-8 text-center transition-all duration-700 delay-300 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <h3 className="font-display text-xl md:text-2xl font-bold text-white mb-4">
              Cotiza directamente con nuestro equipo
            </h3>
